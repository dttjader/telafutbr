...
export default function AdminPartidaEventos() {
  ...
  const load = useCallback(() => {
    fetch('/api/partidas').then(r=>r.json()).then((ps: Partida[]) => setPartida(ps.find(p=>p.id===id)||null));
  ...
  const save = async (updated: Partida) => {
    const r = await fetch('/api/partidas', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(updated) });
    if(r.ok){ flash(true,'Salvo!'); load(); } else flash(false,'Erro ao salvar.');
  };
  ...
  const todosJog = [...jogCasa, ...jogVis];
  const nomeJog = (jid: string) => jogadores.find(j=>j.id===jid)?.nome ?? jid;
  const nomeTime = (tid: string) => times.find(t=>t.id===tid)?.sigla ?? tid;

  // ── ESCALAÇÃO ──
  const EscalacaoTab = () => {
    const addJogEsc = (timeId: string, isCasa: boolean) => {
      const jogLista = isCasa ? jogCasa : jogVis;
      const novo: EscalacaoJogador = { jogador_id: jogLista[0]?.id ?? '', numero: 1, posicao: 'ATA', titular: true };
      const updated = { ...partida };
      if(isCasa) updated.escalacao_casa = [...updated.escalacao_casa, novo];
      else updated.escalacao_visitante = [...updated.escalacao_visitante, novo];
      save(updated);
    };
    const updateEsc = (isCasa: boolean, idx: number, field: string, value: string|boolean) => {
      const updated = { ...partida };
      const esc = isCasa ? [...updated.escalacao_casa] : [...updated.escalacao_visitante];
      esc[idx] = { ...esc[idx], [field]: field==='numero' ? +value : value };
      if(isCasa) updated.escalacao_casa = esc; else updated.escalacao_visitante = esc;
      save(updated);
    };
    const removeEsc = (isCasa: boolean, idx: number) => {
      const updated = { ...partida };
      if(isCasa) updated.escalacao_casa = updated.escalacao_casa.filter((_,i)=>i!==idx);
      else updated.escalacao_visitante = updated.escalacao_visitante.filter((_,i)=>i!==idx);
      save(updated);
    };

    const EscBlock = ({ isCasa }: { isCasa: boolean }) => {
      ...
      return (
        <div style={{ flex: 1 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'.75rem' }}>
            ...
            <button className="btn btn-primary btn-sm" onClick={()=>addJogEsc(isCasa?partida.time_casa_id:partida.time_visitante_id, isCasa)}>+ Jogador</button>
          </div>
          ...
          {esc.map((e,i)=>(
            <div key={i} style={{ display:'flex', gap:'.5rem', marginBottom:'.4rem', background:'var(--surface2)', borderRadius:6, padding:'.5rem' }}>
              <input type="number" min={1} max={99} value={e.numero} onChange={ev=>updateEsc(isCasa,i,'numero',ev.target.value)}
                style={{width:52,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text)',padding:'.3rem .4rem',textAlign:'center'}} />
              <select value={e.jogador_id} onChange={ev=>updateEsc(isCasa,i,'jogador_id',ev.target.value)}
                style={{flex:1,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text)',padding:'.3rem .4rem'}}>
                {jogLista.map(j=><option key={j.id} value={j.id}>{j.nome}</option>)}
              </select>
              <select value={e.posicao} onChange={ev=>updateEsc(isCasa,i,'posicao',ev.target.value)}
                style={{width:60,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text)',padding:'.3rem .4rem'}}>
                {POSICOES.map(p=><option key={p} value={p}>{p}</option>)}
              </select>
              <label style={{display:'flex',alignItems:'center',gap:4,fontSize:'.8rem',color:'var(--text-muted)',cursor:'pointer'}}>
                <input type="checkbox" checked={e.titular} onChange={ev=>updateEsc(isCasa,i,'titular',ev.target.checked)} /> Titular
              </label>
              <button className="btn btn-danger btn-sm" onClick={()=>removeEsc(isCasa,i)}>✕</button>
            </div>
          ))}
        </div>
      );
    ...
    return (
      <div style={{display:'flex',gap:'2rem',flexWrap:'wrap'}}>
        <EscBlock isCasa={true} />
        <EscBlock isCasa={false} />
      </div>
    );
  };
  ...
  const GolsTab = () => {
    ...
    const goleiros = todosJog.filter(j=>j.posicao==='GOL');
        ...
        <div className="card" style={{marginBottom:'1.5rem'}}>
          ...
          <div className="grid-3">
            <div className="form-group">
              <label>Minuto *</label>
              <input type="number" min={1} max={90} value={form.minuto} onChange={f('minuto')} placeholder="Ex: 23" />
            </div>
            <div className="form-group">
              <label>Acréscimo</label>
              <input type="number" min={0} value={form.acrescimo} onChange={f('acrescimo')} />
            </div>
            <div className="form-group">
              <label>Tipo</label>
            ...
            <div className="form-group" style={{gridColumn:'1/-1'}}>
              ...
              <input value={form.descricao} onChange={f('descricao')} placeholder="Ex: Cabeceio após cruzamento pela esquerda" />
            </div>
          </div>
      ...
      <div>
        <div className="card" style={{marginBottom:'1.5rem'}}>
          ...
          <div className="grid-2" style={{maxWidth:600}}>
            <div className="form-group">
              <label>Minuto *</label>
              <input type="number" min={1} max={90} value={form.minuto} onChange={f('minuto')} />
            </div>
            <div className="form-group">
              <label>Tipo</label>
              <select value={form.tipo} onChange={f('tipo')}>
            ...
            <div className="form-group" style={{gridColumn:'1/-1'}}>
              <label>Motivo</label>
              <input value={form.motivo} onChange={f('motivo')} placeholder="Ex: Falta tática, Reclamação..." />
            </div>
          </div>
  ...
  const tabs = [
    ...
    { key:'gols', label:`⚽ Gols (${partida.gols.length})` },
    { key:'cartoes', label:`🟨 Cartões (${partida.cartoes.length})` },
    { key:'subs', label:`🔄 Subs (${partida.substituicoes.length})` },
  ];
    ...
    <div className="container" style={{paddingTop:'2rem'}}>
      ...
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'1.25rem',marginBottom:'1.5rem',textAlign:'center'}}>
        ...
        <div style={{fontSize:'.75rem',color:'var(--text-muted)',marginTop:'.3rem'}}>
          +{partida.acrescimo_primeiro}min / +{partida.acrescimo_segundo}min
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:'.25rem',marginBottom:'1.5rem',borderBottom:'1px solid var(--border)',paddingBottom:'.5rem',flexWrap:'wrap'}}>
        {tabs.map(t=>(
          <button key={t.key} onClick={()=>setTab(t.key as typeof tab)}
            style={{padding:'.4rem 1rem',fontFamily:"'Bebas Neue',sans-serif",fontSize:'.95rem',letterSpacing:'.05em',border:'none',borderRadius:6,cursor:'pointer',transition:'all .15s',
              background: tab===t.key ? 'var(--verde)' : 'transparent',
