  ...
  return (
    ...
    <div className="container" style={{paddingTop:'2rem'}}>
      ...
      <div className="card" style={{marginBottom:'2rem'}}>
        <h2 style={{fontSize:'1.3rem',marginBottom:'1.25rem',color:'var(--amarelo)'}}>{editId?'✏️ Editar Jogador':'+ Novo Jogador'}</h2>
        <form onSubmit={submit}>
          <div className="grid-3">
            ...
            <div className="form-group"><label>Número</label><input type="number" min={1} max={99} value={form.numero} onChange={e=>setForm(f=>({...f,numero:e.target.value}))} /></div>
            <div className="form-group"><label>Idade</label><input type="number" min={14} max={50} value={form.idade} onChange={e=>setForm(f=>({...f,idade:e.target.value}))} placeholder="Ex: 28" /></div>
            <div className="form-group"><label>Nacionalidade</label>
              <select value={form.nacionalidade} onChange={e=>setForm(f=>({...f,nacionalidade:e.target.value}))}>
          ...
          <div style={{background:'var(--surface2)',borderRadius:8,padding:'1rem',marginBottom:'1rem'}}>
            <p style={{fontSize:'.8rem',color:'var(--text-muted)',marginBottom:'.75rem',textTransform:'uppercase',letterSpacing:'.08em',fontWeight:700}}>{editId?'🔄 Transferência':'🏠 Time inicial'}</p>
            <div className="grid-3">
              {!editId&&(
                <>
                  <div className="form-group" style={{margin:0}}><label>Time *</label>
              ...
              {editId&&(
                <>
                  <div className="form-group" style={{margin:0}}><label>Novo time</label>
          ...
          <div style={{display:'flex',gap:'.75rem'}}>
            <button type="submit" className="btn btn-primary" disabled={loading}>{loading?'Salvando...':(editId?'Salvar alterações':'Cadastrar jogador')}</button>
