...
interface Props {
  ...
  rankingArbitros: EntradaArbitro[];
...
export function DadosClient({
  ...
  placaresFrequentes,
  rankingEstadio, rankingEstado, rankingArbitros,
  ...
  // Subdivisão dos técnicos
  // limiar de "rodadas com atuação suficiente" = 50% das rodadas cadastradas
  const limiar50pct = Math.ceil(totalRodadas * 0.5);

  const tecConsolidados = rankingTecnicos.filter(r => r.j >= limiar50pct);
  const tecIntermediarios = rankingTecnicos.filter(r => r.j > 3 && r.j < limiar50pct);
  ...
  const TabelaTecnicos = ({ dados, subtitulo }: { dados: RankingTecnico[]; subtitulo: string }) => {
    if (dados.length === 0) return (
      <div style={{ marginBottom: '1.5rem' }}>
        <p style={{ fontSize: '.82rem', color: 'var(--text-muted)', fontStyle: 'italic', padding: '.5rem 0' }}>
  ...
  return (
    <div style={{ paddingBottom: '4rem' }}>
      ...
      <div className="container">
        ...
        {/* Árbitros — com seletor de cargo */}
        {card(
          <>
            {/* Cabeçalho + seletor */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem', paddingBottom: '.75rem', borderBottom: '1px solid var(--border)' }}>
              <h2 style={{ fontSize: '1.5rem', margin: 0 }}>🟢 Gols e Cartões por Árbitro</h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem' }}>
                <span style={{ fontSize: '.78rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>Filtrar cargo:</span>
                  ...
                  {CARGO_OPTS.map(opt => (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: '.75rem',
                        cursor: 'pointer',
                        fontWeight: filtroArbitro === opt.value ? 700 : 400,
                        transition: 'all .15s',
                        whiteSpace: 'nowrap',
                      }}
                    >
            ...
            {/* Badge do cargo selecionado */}
            {filtroArbitro && (
              <div style={{ marginBottom: '.75rem', fontSize: '.78rem', color: 'var(--text-muted)' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '.85rem' }}>
                <thead style={{ background: 'var(--surface2)', borderBottom: '2px solid var(--verde)' }}>
                  <tr>
                    {(['Árbitro', 'Cargo', 'Jogos', 'Gols', 'Gols/jogo', '🟨', '🟨/jogo', '🟥', '🟥/jogo'] as const).map(h => (
                      <th key={h} style={{ padding: '.6rem .9rem', textAlign: h === 'Árbitro' || h === 'Cargo' ? 'left' : 'center', fontFamily: "'Bebas Neue',sans-serif", fontSize: '.85rem', letterSpacing: '.06em', color: 'var(--text-muted)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {arbFiltrados.map((a, i) => (
                    <tr key={`${a.cargo}-${a.nome}`} style={{ borderBottom: '1px solid #1a1a1a', background: i % 2 === 0 ? 'var(--surface)' : 'var(--surface2)' }}>
                      ...
                      <td style={{ textAlign: 'center', padding: '.5rem .5rem' }}>{a.g
