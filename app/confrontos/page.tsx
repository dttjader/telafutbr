...
export default function ConfrontosPage() {
  ...
  // Matriz de placares [casa][visitante]
  const placar: ({ gc: number; gv: number } | null)[][] =
  ...
  // Totais gerais (lado esquerdo)
  let totPart = 0, totManVit = 0, totEmp = 0, totVisVit = 0;
  let totGols = 0, totGolsMan = 0, totGolsVis = 0;
  ...
  // Resumo por time (P V E D GM GS Pts)
  const resumo = times.map((time, i) => {
  ...
  const th: React.CSSProperties = {
    border: '1px solid #2a2a2a', textAlign: 'center', padding: '2px 1px',
    fontSize: 9, background: 'var(--surface2)', color: 'var(--text-muted)',
    whiteSpace: 'nowrap', fontFamily: "'Bebas Neue', sans-serif", letterSpacing: '.05em',
  };
  const td: React.CSSProperties = {
    border: '1px solid #222', textAlign: 'center', padding: '2px 0',
    ...
    <div style={{ paddingBottom: '4rem' }}>
      ...
      <div className="container">
          ...
          <table style={{ borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 900 }}>
            ...
            <thead>
              <tr>
                ...
                {times.map(t => (
                  <th key={t.id} style={{ ...th, writingMode: 'vertical-rl', transform: 'rotate(180deg)', height: 56, verticalAlign: 'bottom', padding: '4px 1px', fontSize: 9 }}>
                    {t.id}
                  </th>
                ))}
                {['P','V','E','D','GM','GS','Pts'].map(h => (
                  <th key={h} style={{ ...th, background: '#1a2a1a', color: 'var(--verde)', writingMode: 'vertical-rl', transform: 'rotate(180deg)', height: 56, verticalAlign: 'bottom', padding: '4px 1px' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {times.map((time, i) => {
                ...
                return (
                  <tr key={time.id} style={{ background: i % 2 === 0 ? 'var(--surface)' : 'var(--surface2)' }}>
                    {/* Nome */}
                    <td style={{ ...td, background: 'var(--surface2)', fontFamily: "'Bebas Neue',sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: '.04em', position: 'sticky', left: 0, zIndex: 1, color: 'var(--text)' }}>
                      {time.id}
                    </td>
                    {/* Células de confronto */}
                    {times.map((_, j) => {
                      if (i === j) return (
                        <td key={j} style={{ ...td, background: '#1a1a1a' }}>—</td>
                      );
                      const p = placar[i][j];
                      if (!p) return <td key={j} style={{ ...td, color: '#333' }}></td>;
                      return (
                        <td key={j} style={{ ...td, ...cellStyle(p.gc, p.gv) }}>
                          {p.gc}×{p.gv}
                        </td>
                      );
                    })}
        ...
        <div style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {[
            { titulo: '📋 Resumo como Mandante', isCasa: true },
            { titulo: '✈️ Resumo como Visitante', isCasa: false },
          ].map(({ titulo, isCasa }) => (
                ...
                <tbody>
                  {times.map((time, i) => {
                    ...
                    for (let j = 0; j < n; j++) {
                      ...
                      if (!p) continue;
                      j2++;
                      const gcT = isCasa ? p.gc : p.gv;
                      const gvT = isCasa ? p.gv : p.gc;
                      gm += gcT; gs += gvT;
                    ...
                    return (
                      <tr key={time.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                        ...
                        <td style={{ textAlign: 'center', padding: '4px 4px', fontFamily: "'Bebas Neue',sans-serif", fontSize: 13, color: 'var(--amarelo)' }}>{pts}</td>
                      </tr>
                    );
                  })}
                </tbody>
