...
export default async function JogadorPerfilPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  for (const p of encerradas) {
    ...
    for (const g of p.gols) {
      if (g.jogador_id === id) {
        eventos.push({
          tipo: g.tipo === 'contra' ? 'gol_contra' : 'gol',
          minuto: g.minuto,
          acrescimo: g.acrescimo,
          descricao: g.tipo === 'contra' ? 'Gol contra'
            : g.tipo === 'penalti' ? 'Gol de pênalti'
            : g.tipo === 'falta' ? 'Gol de falta'
            : 'Gol',
        });
      }
      if (g.assistencia_id === id) {
        eventos.push({
          ...
          minuto: g.minuto,
          acrescimo: g.acrescimo,
          descricao: `Assistência para ${jogadores.find(j => j.id === g.jogador_id)?.nome ?? '?'}`,
        });
      }
    }
    ...
    for (const c of p.cartoes) {
      if (c.jogador_id === id) {
        eventos.push({
          ...
          minuto: c.minuto,
          descricao: c.motivo || (c.tipo === 'amarelo' ? 'Cartão amarelo' : 'Cartão vermelho'),
        });
      }
    }
  ...
  const eventoIcon: Record<string, string> = {
    entrada: '↑', saida: '↓', gol: '⚽', gol_contra: '🔴',
    assistencia: '🎯', amarelo: '🟨', vermelho: '🟥',
  };
  const eventoColor: Record<string, string> = {
    entrada: 'var(--libertadores)', saida: 'var(--rebaixamento)',
    ...
    <div style={{ paddingBottom: '4rem' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0a0a0a 0%,#0d1f0d 50%,#0a0a0a 100%)', borderBottom: '1px solid var(--border)', padding: '2.5rem 0 2rem', marginBottom: '2rem' }}>
        <div className="container">
          <Link href="/analitico" style={{ fontSize: '.78rem', color: 'var(--verde)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '.3rem', marginBottom: '1rem' }}>
            ← Voltar ao Analítico
          </Link>
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {historicoPartidas.map(({ partida: p, timeCasa, timeVis, estadio, titular, minutos, minutoEntrada, minutoSaida, eventos }) => {
            ...
            const acr2 = p.acrescimo_segundo ?? 0;
            const totalPartida = 45 + acr1 + 45 + acr2;
            const jogadorEhCasa = p.time_casa_id === jogador.time_atual ||
              p.escalacao_casa.some((e: any) => e.jogador_id === id);
            ...
            return (
              <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '.75rem 1.25rem', borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
                  ...
                  <div style={{ position: 'relative', height: 8, background: '#222', borderRadius: 4, overflow: 'visible' }}>
                    ...
                    {eventos.map((ev, ei) => {
                      ...
                      return (
                        <div key={ei} title={`${ev.minuto}' - ${ev.descricao}`} style={{
                          position: 'absolute',
                          ...
                          width: 14, height: 14,
                          borderRadius: '50%',
                          background: eventoColor[ev.tipo],
                ...
                {eventos.length > 0 ? (
                  <div style={{ padding: '.75rem 1.25rem', display: 'flex', flexWrap: 'wrap', gap: '.5rem' }}>
                    {eventos.map((ev, ei) => (
                      <div key={ei} style={{
                        ...
                        background: 'var(--surface2)', border: `1px solid ${eventoColor[ev.tipo]}33`,
                        borderRadius: 6, padding: '.3rem .65rem', fontSize: '.8rem',
                      }}>
                        <span>{eventoIcon[ev.tipo]}</span>
                        <span style={{ fontFamily: "'Bebas Neue',sans-serif", color: eventoColor[ev.tipo], fontSize: '.95rem' }}>
                  ...
                  <div style={{ padding: '.75rem 1.25rem', fontSize: '.8rem', color: 'var(--text-muted)' }}>
                    Sem eventos registrados nesta partida.
                  </div>
                )}
              </div>
            );
